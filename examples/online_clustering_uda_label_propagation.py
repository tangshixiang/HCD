...
def main_worker(args):
    ...
    P_LP=p_lp(alpha=0.99,method=args.point_method,connect_num=args.connect_num,topk_num=args.topk_num)
    S_LP=s_lp(alpha=0.99,topk_num=args.topk_num,method=args.method)
    C_LP=c_lp(alpha=0.99,topk_num=args.topk_num,method=args.method,point_wei=args.merge_p_wei,connect_num=args.connect_num)
    Split_LP=split_lp(args.split_alpha,args.split_num,args.split_anchor_thre,connect_num=args.connect_num)
    ...
    #for epoch in range(args.epochs):
        ...
        if ((all_iters//(2.5*args.iters)-eval_cnt>0) and all_iters>=15*args.iters) or (all_iters>=60*args.iters):
            ...
            if is_best:
                try:
                    torch.save(memory.t_sub_label[memory.source_classes:].cpu(),osp.join(args.logs_dir,'label_0.pth.tar'))
                    torch.save(memory.labels[memory.source_classes:].cpu(),osp.join(args.logs_dir,'label_1.pth.tar'))
                except:
                    pass
