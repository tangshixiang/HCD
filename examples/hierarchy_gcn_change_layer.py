...
class Sub_Cluster_Level_GCN(nn.Module):
    def __init__(self, feature_dim, nhid, feature_size,source_classes,nclass, momentum=0.2,dropout=0,cal_num=30):
        ...
        self.conv1 = GraphConv(feature_dim, nhid, MeanAggregator, dropout)
        #self.conv2 = GraphConv(nhid, 1024, MeanAggregator, dropout)
...
class Hierarchy_GCN(object):
    def __init__(self,point_level_gcn,sub_cluster_level_gcn,cluster_level_gcn,utils,r=1,merge_label_func=0,point_add_dyn_thre=0,
                ...
                consider_cur_batch=0,num_penalty=1,inference_thre=[0.65,0.55,0.55,0.45],num_penalty_ratio=1,fix_dyn_thre=0,pred_ratio=1,pred_dyn_type=0,neighbor_num=64):
    ...
    def inference(self,t_indexes,memory,infer):
        if infer:
            ...
            all_pred_sub,sub_inter_sim,self.sub_sim,self.sub_nums=self.sub_cluster_level_gcn(t_indexes,memory.features,memory.labels,memory.t_sub_label,infer,ori_0,ori_knn_neighbor,all_pred,output_feat=True,min_point=self.mean_min_points,num_penalty=self.num_penalty,num_penalty_ratio=self.num_penalty_ratio,pred_ratio=self.pred_ratio)
    ...
    def point_level_merge_split(self,indexes,all_pred,ori_knn_neighbor,memory):
        ...
        #self.sub_nums=nums
        try:
            self.sub_sim=self.sub_sim.mm(memory.features[indexes].t())
