...
from spcl.models.hm_final import HybridMemory
...
from spcl.models.hierarchy_lp_final import Hierarchy_GCN,Utils,p_lp,s_lp,c_lp,split_lp
...
def main_worker(args):
    ...
    memory = HybridMemory(model.module.num_features, num_samp,source_classes,source_sam,
                            ...
                            cluster_k2=args.cluster_k2,iterative=args.iterative).cuda()
    ...
    #save gt label
    # print('save...')
    # torch.save(torch.tensor(source_label),'true_lab/{}.pth.tar'.format(args.dataset_source))
    # print('ok')
    # import pdb;pdb.set_trace()
    ...
    P_LP=p_lp(alpha=args.merge_alpha,method=args.point_method,topk_num=args.topk_num)
    S_LP=s_lp(alpha=args.merge_alpha,topk_num=args.topk_num,method=args.method)
    C_LP=c_lp(alpha=args.merge_alpha,topk_num=args.topk_num,method=args.method,point_wei=args.merge_p_wei,connect_num=args.connect_num)
    Split_LP=split_lp(args.split_alpha,args.split_num,args.split_anchor_thre,connect_num=args.connect_num,gap=args.split_gap,sp_method=args.sp_method)
    hier_gcn=Hierarchy_GCN(P_LP,S_LP,C_LP,Split_LP,uti,thre=args.thre,neighbor_num=args.neighbor_num,merge_wo_outlier=args.merge_wo_outlier,
                           jaccard_debug=args.jaccard_debug,nei_tgt_only=args.nei_tgt_only)

    # Trainer
    trainer = SpCLTrainer_UDA(model, memory, src_nums,writer,hier_gcn,len(dataset_target.train),len(dataset_source.train),update_iters=args.iters,update_method=args.update_method)
    ...
    #for epoch in range(args.epochs):
        ...
        if (epoch==0):#initialize
            ...
            #convert pseudo label-->mapping

            for i in range(args.iterative):
                #memory.labels[i]=torch.cat((torch.arange(source_classes), torch.arange(len(pseudo_labels))+source_classes)).cuda()
                memory.labels[i] = torch.cat((torch.arange(source_classes), pseudo_labels)).cuda()
            #memory.labels[-1] = torch.cat((torch.arange(source_classes), pseudo_labels)).cuda()
        else:
            ...
            pseudo_labeled_dataset = []
            pseudo_labels=memory.labels[-1][src_nums:].cpu()
            #pseudo_labels=memory.label_cache[source_classes:].cpu()
            for i, ((fname, _, cid), label) in enumerate(zip(sorted(dataset_target.train), pseudo_labels)):
                pseudo_labeled_dataset.append((fname,label.item(),cid))

        pseudo_labels=memory.labels[-1][src_nums:]
        ...
        sub_cluster_label=memory.labels[0][source_classes:].tolist()

        if args.num_instances>0:
            #iters=int(((index2label>1).sum()*4+(index2label==1).sum()+(index2label[(index2label>4)]-4).sum())//(args.batch_size))+10
            #iters=int(((index2label>3).sum()*4+(index2label==3).sum()*3+(index2label==2).sum()*2+(index2label==1).sum())//(args.batch_size))+1
        ...
        trainer.train(epoch, train_loader_source, train_loader_target, optimizer,
                    ...
                    lr_scheduler=lr_scheduler,update_before=args.update_before,args=args)
        all_iters+=iters
        if ((all_iters//(2.5*args.iters)-eval_cnt>0) and all_iters<10*args.iters):
           eval_cnt+=1
        if ((all_iters//(2.5*args.iters)-eval_cnt>0) and all_iters>=10*args.iters) or (all_iters>=60*args.iters):
            eval_cnt+=1
       # if ((epoch+1)%args.eval_step==0 or (epoch==args.epochs-1)):
        #if ((all_iters//(1.0*args.iters)-eval_cnt>0)):
            ...
            if is_best:
                for kk in range(args.iterative):
                    torch.save(memory.labels[kk][memory.source_classes:].cpu(),osp.join(args.logs_dir,'label_{}.pth.tar'.format(kk)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--thre', type=float, nargs='+',default=[0.25,0.25,0.25])
    parser.add_argument('--split-num', type=int,default=8)
    parser.add_argument('--topk-num', type=float,default=0.45) #avg linkage thre
    parser.add_argument('--neighbor-num', type=int, default=64)
    parser.add_argument('--method', type=int, default=14)
    parser.add_argument('--merge-wo-outlier', type=int, default=0)
    parser.add_argument('--merge-p-wei', type=float, default=0.0)
    parser.add_argument('--merge-alpha', type=float, default=0.99)
    parser.add_argument('--split-gap', type=int, default=1)
    parser.add_argument('--split-alpha', type=float, default=0.3)
    parser.add_argument('--split-anchor-thre', type=float, default=0.6)
    parser.add_argument('--point-method', type=int, default=1)
    parser.add_argument('--jaccard-debug', type=int, default=-1)
    parser.add_argument('--update-method', type=int, default=1)#for trainer
    parser.add_argument('--connect-num', type=int, default=20)
    parser.add_argument('--sp-method', type=int)

    parser.add_argument('--iterative',type=int,default=2)
    parser.add_argument('--update-before', type=int, default=0)
    parser.add_argument('--nei-tgt-only', type=int, default=0)
