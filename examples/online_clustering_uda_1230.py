...
from examples.hierarchy_gcn_1230 import Hierarchy_GCN,Utils,p_gcn,s_gcn,c_gcn,split_lp
...
def main_worker(args):
    ...
    S_GCN=s_gcn(2048,2048,source_classes,source_classes,cal_num=args.gcn_cal_num,A_thre=args.A_thre)
    C_GCN=c_gcn(2048,2048,source_classes,source_classes,cal_num=args.gcn_cal_num,A_thre=args.A_thre)
    Split_LP=split_lp(0.5)
    hier_gcn=Hierarchy_GCN(P_GCN,S_GCN,C_GCN,Split_LP,uti,r=args.gcn_r,point_add_dyn_thre=args.gcn_point_add_dyn_thre,
            ...
            num_penalty_ratio=args.gcn_num_penalty_ratio,fix_dyn_thre=args.fix_dyn_thre,
            pred_ratio=args.gcn_pred_ratio,pred_dyn_type=args.gcn_pred_dyn_type,neighbor_num=args.gcn_neighbor_num,split_type=args.split_type,
            thre=args.thre)
    ...
    #params_split_lp=[{"params":[value]} for _, value in Split_LP.named_parameters() if value.requires_grad]
    ...
    #optimizer_split_lp=torch.optim.SGD(params_split_lp, lr=args.lr_gcn, weight_decay=args.weight_decay)
    ...
    #lr_split_lp=torch.optim.lr_scheduler.StepLR(optimizer_split_lp, step_size=args.step_size_gcn, gamma=0.1)
    optimizer_gcn=[optimizer_p_gcn,optimizer_s_gcn,optimizer_c_gcn]#,optimizer_split_lp]
    lr_gcn=[lr_p_gcn,lr_s_gcn,lr_c_gcn]#,lr_split_lp]
    ...
    #for epoch in range(args.epochs):
        ...
        if (epoch==0):#initialize
            ...
            if not args.src_feat:
                ...
                memory.t_sub_label=torch.arange(source_classes+len(dataset_target.train)).cuda()#torch.cat((torch.arange(source_classes), pseudo_labels)).cuda()
        ...
        trainer.train(epoch, train_loader_source, train_loader_target, optimizer,
                    ...
                    lr_scheduler=lr_scheduler,lr_scheduler_gcn=lr_gcn,optimizer_gcn=optimizer_gcn,gcn_iters=args.gcn_iters,update_tgt=0)
        all_iters+=iters
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gcn-merge-func', type=int, default=2)
    ...
    parser.add_argument('--gcn-cal-num', type=int, default=48)
    parser.add_argument('--gcn-cluster-level-thre', type=float, default=0.5)
    parser.add_argument('--gcn-num-penalty', type=int, default=1)
    parser.add_argument('--gcn-num-penalty-ratio', type=float, default=1)
    parser.add_argument('--gcn-neighbor-num', type=int, default=80)
    parser.add_argument('--gcn-pred-ratio', type=float, default=0.1)
    parser.add_argument('--gcn-pred-dyn-type', type=int, default=1)
    ...
    parser.add_argument('--A-thre', type=float, default=0.3)
    # path
