...
from spcl.trainers_online_wo_src_cen import SpCLTrainer_UDA
...
from examples.hierarchy_gcn_wo_src_cen import Hierarchy_GCN,Utils,p_gcn,s_gcn,c_gcn
...
def main_worker(args):
    ...
    num_samp=len(dataset_source.train)+len(dataset_target.train)
    ...
    memory.labels[:len(dataset_source.train)]=torch.tensor(source_label).cuda()
    memory.sub_label[:len(dataset_source.train)]=torch.tensor(source_label).cuda()
    memory.features[:len(dataset_source.train)]=torch.stack(all_source_feature,0).cuda()
    src_nums=len(dataset_source.train)
    ...
    memory.features[src_nums:]=F.normalize(target_features, dim=1)
    del tgt_cluster_loader, target_features, sour_cluster_loader, sour_fea_dict
    ...
    hier_gcn=Hierarchy_GCN(P_GCN,S_GCN,C_GCN,uti,r=args.gcn_r,point_add_dyn_thre=args.gcn_point_add_dyn_thre,
            ...
            cal_num=args.gcn_cal_num,tgt_only=args.tgt_only,dyn_thre_type=args.dyn_thre_type)
    ...
    # Trainer
    trainer = SpCLTrainer_UDA(model, memory, src_nums,writer,hier_gcn,len(dataset_target.train),len(dataset_source.train))

    target_cnt=np.zeros(len(memory.features[src_nums:])).astype('int')
    all_iters=0
    epoch=-1
    eval_cnt=0
    while(all_iters<args.epochs*args.iters):
    #for epoch in range(args.epochs):
        ...
        if (epoch==0):#initialize
            ...
            memory.sub_label[src_nums:]=(torch.arange(len(pseudo_labels))+src_nums).cuda()
        else:
        ...
        all_iters+=iters
        if (all_iters//1000-eval_cnt>0) or (all_iters>=args.epochs*args.iters):
            eval_cnt+=1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--src-feat', type=int, default=1)
