...
def main_worker(args):
    ...
    source_sam=0
    num_samp=len(dataset_target.train)
    ...
    hier_gcn=Hierarchy_GCN(P_LP,S_LP,C_LP,Split_LP,uti,thre=args.thre,neighbor_num=args.neighbor_num,merge_wo_outlier=args.merge_wo_outlier,jaccard_debug=args.jaccard_debug)

    # Trainer
    trainer = SpCLTrainer_USL(model, memory,hier_gcn,len(dataset_target.train),update_iters=args.iters,update_method=args.update_method)
    ...
    #for epoch in range(args.epochs):
        ...
        if (epoch==0):#initialize
            ...
            pseudo_labeled_dataset = []
            pseudo_labels=memory.labels[-1].cpu()
            #pseudo_labels=memory.label_cache[source_classes:].cpu()
            for i, ((fname, _, cid), label) in enumerate(zip(sorted(dataset_target.train), pseudo_labels)):
                pseudo_labeled_dataset.append((fname,label.item(),cid))

        pseudo_labels=memory.labels[-1]
        ...
        trainer.train(epoch, train_loader_target, optimizer,
                    ...
                    lr_scheduler=lr_scheduler,update_before=args.update_before)
        all_iters+=iters
        if ((all_iters//(2.5*args.iters)-eval_cnt>0) and all_iters<5*args.iters):
            eval_cnt+=1
        if ((all_iters//(2.5*args.iters)-eval_cnt>0) and all_iters>=5*args.iters) or (all_iters>=60*args.iters):
            eval_cnt+=1
