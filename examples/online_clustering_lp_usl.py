...
from spcl.trainers_online_usl import SpCLTrainer_USL
...
def main_worker(args):
    ...
    source_sam=0
    num_samp=source_classes+len(dataset_target.train)
    ...
    hier_lp=Hierarchy_LP(P_LP,Merge_LP,Split_LP,uti,neighbor_num=args.neighbor_num,
            ...
            merge_type=args.merge_type,point_bias=args.point_bias)

    # Trainer
    trainer = SpCLTrainer_USL(model, memory, src_nums,writer,hier_lp,len(dataset_target.train),0,update_iters=args.iters)
    ...
    #for epoch in range(args.epochs):
        ...
        if (epoch==0):#initialize
            ...
            for itera in range(args.iterative_num):
                memory.labels[itera]=pseudo_labels.clone().cuda()#torch.arange(len(pseudo_labels)).cuda()
            #memory.labels[-1]= pseudo_labels.cuda()
        ...
        sub_cluster_label=memory.labels[0].tolist()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--merge-method', type=int, default=2)
