...
from spcl.models.hm_lp import HybridMemory
from spcl.models.hierarchy_lp import Hierarchy_LP,Utils,split_lp,merge_lp,p_lp
from spcl.trainers_online_label_propagation_v2 import SpCLTrainer_UDA
...
def main_worker(args):
    ...
    memory = HybridMemory(model.module.num_features, num_samp,source_classes,source_sam,
                            temp=args.temp, momentum=args.momentum,iterative=args.iterative_num).cuda()
    ...
    P_LP=p_lp()
    Merge_LP=merge_lp(args.merge_alpha,args.topk_num,args.merge_temp)
    Split_LP=split_lp(args.split_alpha,args.split_num,args.split_confidence_thre)
    hier_lp=Hierarchy_LP(P_LP,Merge_LP,Split_LP,uti,neighbor_num=args.neighbor_num,
            merge_wo_outlier=args.merge_wo_outlier,start_label_num=num_samp,merge_method=args.merge_method,
            merge_type=args.merge_type,point_bias=args.point_bias,method5_ratio=args.method5_ratio)

    # Trainer
    trainer = SpCLTrainer_UDA(model, memory, src_nums,writer,hier_lp,len(dataset_target.train),len(dataset_source.train),update_iters=args.iters)
    ...
    #for epoch in range(args.epochs):
        ...
        if (epoch==0):#initialize
                ...
                if debug:
                    ...
                    for i, ((fname, pid, cid), id) in enumerate(zip(sorted(dataset_target.train), cluster_id)):
                        if id!=-1:
                            ...
                            outliers += 1
                        hier_lp.debug_label.append(pid)
                    hier_lp.debug_label=torch.tensor(hier_lp.debug_label).cuda()
                else:
            ...
            for itera in range(args.iterative_num):
                memory.labels[itera]=torch.arange(source_classes+len(pseudo_labels)).cuda()
                #memory.labels[itera] = torch.cat((torch.arange(source_classes), pseudo_labels)).cuda()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--thre', type=float, nargs='+',default=[0.06,0.15,0.1])
    parser.add_argument('--split-num', type=int,default=8)
    parser.add_argument('--topk-num', type=int,default=15)
    parser.add_argument('--neighbor-num', type=int, default=48)
    parser.add_argument('--merge-wo-outlier', type=int, default=0)
    parser.add_argument('--split-alpha', type=float, default=0.99)
    parser.add_argument('--split-confidence-thre', type=float, default=0.7)
    parser.add_argument('--iterative-num', type=int, default=2)
    parser.add_argument('--merge-temp', type=float, default=0.05)
    parser.add_argument('--merge-alpha', type=float, default=0.99)
    parser.add_argument('--merge-method', type=int, default=5)
    parser.add_argument('--merge-type', type=int, default=4) #inter
    parser.add_argument('--point-bias', type=float, default=0.7)
    parser.add_argument('--method5-ratio', type=float, default=0.85)
