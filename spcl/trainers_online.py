...
class SpCLTrainer_UDA(object):
    def __init__(self, encoder, memory, source_classes,writer,hierarchy_gcn,target_samples,source_samples,update_iters=400):
    ...
    def train(self, epoch, data_loader_source, data_loader_target,
                    optimizer, print_freq=10, train_iters=400,all_iters=-1,lr_scheduler=None,lr_scheduler_gcn=None,optimizer_gcn=None,pretrain=0,train_gcn=0,only_update_label=0,gcn_iters=400,update_tgt=1):
        self.encoder.train()
        ...
        for i in range(train_iters):
            ...
            if update_tgt and epoch==1 and i==0:
            ...
            if (all_iters !=-1):
                if (all_iters+i)%self.update_iters==0 and all_iters!=0:
                    ...
                    lr_scheduler.step()
                if (all_iters+i)%gcn_iters==0 and all_iters!=0:
                    print('{}----update gcn lr scheduler--------'.format((all_iters+i)//gcn_iters))
