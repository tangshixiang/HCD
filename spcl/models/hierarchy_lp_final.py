...
class Point_Level_LP(nn.Module):
    ...
    def forward(self,indexes,features,neighbor_num,ori_0,ori_knn_neighbor,gt_conf=None,f_s=None,train=0,two_hop=0):
        ...
        if self.method==3:#ori method
            ...
            preds = torch.matmul(
                torch.inverse(torch.eye(neighbor_num).unsqueeze(0).expand_as(W).cuda() - self.alpha * W + self.eps), Y)
...
class Cluster_Level_LP(nn.Module):
    ...
    def forward(self, indexes, features,neighbor_num0,ori_0,ori_knn_neighbor,gt_conf_ori=None,f_s=None,train=0,labels=None,gt_label=None,debug_label=None,bias=0,step=2,point_W=None,two_hop=0,memory=None,point_pred=None):
        ...
        if self.method==19:
            ...
            #cal avg
            avg_thre=torch.mean(point_W[:,0],dim=-1,keepdim=True)+torch.std(point_W[:,0],dim=-1,keepdim=True)
            print('avg thre:',avg_thre.view(-1))
            # mask_top = (W0 > self.topk_num).long()  # thre
            W *= ((point_W > avg_thre.unsqueeze(2).expand_as(point_W)).long())
            ...
            W = D1 * W * D2
            W *= masks.expand_as(W)
        if self.method == 18:  # for ablation study-->avg linkage
            #Y = torch.zeros(bs, neighbor_num, neighbor_num).cuda()
            indices = []
            mapping = []
            # for debug
            ...
            tmp = []
            for i in range(bs):
                indices.append(torch.arange(neighbor_num).cuda())
                mapping.append(gt_label[i])
                tmp.append(gt_label[i].tolist())

            indes = []
            for i in range(bs):
                indes.append(np.arange(1,neighbor_num))

            i_feat = clu_sum[labels[ori_knn_neighbor.view(-1)]].view(bs, -1, 2048)
            W0 = i_feat.bmm(i_feat.permute(0, 2, 1))

            preds=torch.zeros((bs,neighbor_num,neighbor_num)).cuda()
            preds[:,0]=(W0[:,0] > self.topk_num).float()
        ...
        if self.method==16:
            ...
            W = D1 * W * D2
            W *= masks.expand_as(W)
        if self.method==15: #cluster thre
            Y = torch.zeros(bs, neighbor_num, neighbor_num).cuda()
            indices = []
            mapping = []
            # for debug
...
class Split_LP(nn.Module):
    def __init__(self,alpha,split_num,anchor_thre,gap,connect_num=30,sp_method=10):
        super(Split_LP, self).__init__()
        self.alpha=alpha
        self.eps = np.finfo(float).eps
        self.method=sp_method #new try
    ...
    def forward(self,indexes,features,labels,sub_level=0,sub_labels=None,outliers_label=None,ori_knn_neighbor=None,memory=None,two_hop=0,point_pred=None,point_W=None):
        all_idxs=torch.arange(len(labels)).cuda()
        split_nums=[]
        if self.method==-1: #wo split
            return ori_knn_neighbor
        if self.method == 4:  # method1+anchor thre
            split_num = 0
            if sub_level:
                print_cnts = 0
                for n, idx in enumerate(indexes):
            ...
            if sub_level:
                for n, idx in enumerate(indexes):
                    ...
                    anchor_idx = split_idxs[torch.argmin(split_sim).item()]
                    anchor_idxs.append(anchor_idx)
                    anchor_indices.append(torch.argmin(split_sim).item())

                    for sp in range(2, min(self.split_num,len(split_idxs))):
                        split_sim_2 = features[anchor_idx].unsqueeze(0).mm(split_feat.t())
                        split_sim_2[split_sim_2 < split_sim] = split_sim[split_sim_2 < split_sim]
                        if torch.min(split_sim_2) >= self.anchor_thre:
                            continue
                        anchor_idx = split_idxs[torch.argmin(split_sim_2).item()]
                        anchor_idxs.append(anchor_idx)
                        anchor_indices.append(torch.argmin(split_sim_2).item())
                        split_sim = split_sim_2.clone()
                    ...
                    pred = torch.argmax(pred, dim=1)
                    # lab=torch.tensor([anchor_idx.item(),anchor_idx_2.item()]).cuda()
                    labs = torch.tensor(anchor_idxs).cuda()
                    if sub_labels is None:
                        labels[split_idxs] = labs[pred]
                    else:
                        for sub, pre in zip(split_idxs, pred):
                            labels[batch_idx[batch_sub_label == sub]] = labs[pre]
                    # labels[idx] = idx
            ...
            if sub_level:
                ...
                for n, idx in enumerate(indexes):
                    # reduce duplicate
                    batch_idx = all_idxs[labels == labels[idx]]
                    if len(batch_idx)<=4:#save computation
                        continue
                    # if len(batch_idx)>3000:
                    #     print('------------>3000-----------')
                    #     import pdb;pdb.set_trace()
                    ...
                    for sp in range(2, min(len(split_idxs),self.split_num)):
                        # fix bug 20210116
                        split_sim_2 = features[anchor_idx].unsqueeze(0).mm(split_feat.t())
                        split_sim_2[split_sim_2 < split_sim] = split_sim[split_sim_2 < split_sim]
                        if torch.min(split_sim_2) >= self.anchor_thre:
        ...
        elif self.method==11: #random selecting
            split_num = 0
            if sub_level:
                for n, idx in enumerate(indexes):
                    batch_idx = all_idxs[labels == labels[idx]]
                    if len(batch_idx) <= 4:
                        continue
                    if sub_labels is None:
                        split_idxs = np.array(batch_idx.tolist())
                    else:
                        batch_sub_label = sub_labels[batch_idx]
                        split_idxs, split_ind, split_cnts = np.unique(batch_sub_label.cpu().numpy(), return_index=True,
                                                                      return_counts=True)
                        split_idxs = split_idxs# sub label
                    ...
                    anchor_indices=np.random.choice(np.arange(len(split_idxs)),min(len(split_idxs),self.split_num),replace=False).tolist()
                    anchor_idxs=split_idxs[anchor_indices].tolist() # index self
                    # fix bug 104
                    Y = torch.zeros((len(split_idxs), len(anchor_idxs))).cuda()
                    Y[anchor_indices, torch.arange(len(anchor_idxs))] = 1
                ...
                for n, idx in enumerate(indexes):
                    ...
                    anchor_indices = np.random.choice(np.arange(len(split_idxs)), min(len(split_idxs), self.split_num),
                    ...
                    #     print_cnts=1
                    # split_nums.append([len(split_idxs)-torch.sum(pred).item(),torch.sum(pred).item()])
        elif self.method==12: #density peak
            peak_thresh=0.8
            outlier_thresh=0.4
            split_num = 0
            if sub_level:
                for n, idx in enumerate(indexes):
                    ...
                    similarity=split_feat.mm(split_feat.t())
                    density=torch.sum(similarity,dim=-1)-1
                    higher_nn_sim=[]
                    #import pdb;pdb.set_trace()
                    for i,sim in enumerate(similarity):
                        density_idx=density>density[i]
                        density_idx=density_idx.nonzero().view(-1)
                        if len(density_idx)==0:
                            higher_nn_sim.append(-1)
                        else:
                            #import pdb;pdb.set_trace()
                            father_candidates_sim=sim[density_idx]
                            idx_i=torch.argmax(father_candidates_sim)
                            higher_nn_sim.append(sim[density_idx[idx_i]].item())
                    higher_nn_sim=torch.tensor(higher_nn_sim)
                    not_outlier=(
                        ((higher_nn_sim<peak_thresh) & (higher_nn_sim > outlier_thresh))
                        | (higher_nn_sim<-0.5)
                    )
                    not_outlier=not_outlier.nonzero().view(-1)
                    higher_nn_sim_no_outlier = higher_nn_sim[not_outlier]
                    peaks=torch.argsort(-torch.abs(higher_nn_sim_no_outlier))[:self.split_num]
                    #peaks=not_outlier[peaks]
                    if len(peaks)<=1:
                        continue
                    anchor_indices=peaks.tolist()
                    anchor_idxs = split_idxs[anchor_indices].tolist()
...
class Hierarchy_GCN(object):
    def __init__(self,point_level_lp,sub_cluster_level_lp,cluster_level_lp,split_lp,utils,neighbor_num=64,thre=[0,06.15,0.1],
                debug_label=[],merge_wo_outlier=0,jaccard_debug=0,nei_tgt_only=1):
        ...
        self.nei_tgt_only=nei_tgt_only
    ...
    def inference(self,t_indexes,memory,infer):
        ...
        if self.two_hop:
            ...
            if self.nei_tgt_only:
                ori_0 = compute_knn(memory.features[memory.source_classes:].clone(), k1=self.neighbor_num)
                ori_0+= memory.source_classes
                ori_0=np.concatenate((np.zeros((memory.source_classes,self.neighbor_num)),ori_0)).astype(np.int64)
            else:
        ...
        for i in range(memory.iterative-1):
            ori_labels=memory.labels[i][t_indexes]
            change_sub_label = t_indexes[ori_labels == t_indexes]
            memory.labels[i][t_indexes]=0
            if len(change_sub_label) > 0:
                all_label = torch.arange(len(memory.features)).cuda()
                for change_lab in change_sub_label:
                    change_idx = all_label[memory.labels[i] == change_lab]
                    if len(change_idx) > 0:
                        memory.labels[i][change_idx]=change_idx[0]
            memory.labels[i][t_indexes]=t_indexes

        #step2 change label
        empty_label=set(torch.arange(memory.labels[-1].max()+1).tolist())-set(memory.labels[-1].tolist())
        if len(empty_label)<2*len(t_indexes):
            outliers_label=torch.arange(memory.labels[-1].max()+1,memory.labels[-1].max()+1+2*len(t_indexes)).cuda()
        else:
            empty_label=list(empty_label)[-2*len(t_indexes):]
            outliers_label=torch.tensor(empty_label).cuda()
        memory.labels[-1][t_indexes]=outliers_label[:len(t_indexes)]
        ...
        # sub
        sub_sum=memory.features#start
        for i in range(memory.iterative-1):
            # split sub cluster 1-->2 split gcn
            sub_labels=None if i==0 else memory.labels[i-1]
            point_neighbor = self.split_lp(t_indexes, sub_sum, memory.labels[i], sub_level=1,
                                           ori_knn_neighbor=point_neighbor, two_hop=self.two_hop, point_pred=all_pred,sub_labels=sub_labels)

            all_gt_sub_label=memory.labels[i][point_neighbor.view(-1)].view(len(t_indexes),-1)
            all_pred_sub,sub_sum,_,sub_mapping_0,sub_mapping_1,sub_mapping_2=self.cluster_level_lp(t_indexes,memory.features,self.neighbor_num,ori_0,point_neighbor,f_s=cal_feat,labels=memory.labels[i],gt_label=all_gt_sub_label,
                                                                                                     debug_label=self.debug_label,bias=self.thre[1],step=i,point_W=point_W,two_hop=self.two_hop,memory=memory,point_pred=all_pred)
            self.sub_cluster_level_merge_split(t_indexes,memory,all_pred_sub,point_neighbor,sub_mapping_0,sub_mapping_1,sub_mapping_2,near_neigh,merge_idxs,i)
        ...
        if len(empty_label)<self.split_lp.split_num*len(t_indexes)+1:
            outliers_label=torch.arange(memory.labels[-1].max()+1,memory.labels[-1].max()+2+self.split_lp.split_num*len(t_indexes)).cuda()
        else:
            empty_label=list(empty_label)[-(1+self.split_lp.split_num*len(t_indexes)):]
            outliers_label=torch.tensor(empty_label).cuda()

        sub_labels=memory.labels[-2] if memory.iterative>1 else torch.arange(len(memory.features)).long().cuda()
        point_neighbor=self.split_lp(t_indexes,sub_sum,memory.labels[-1],sub_level=0,sub_labels=sub_labels,outliers_label=outliers_label,ori_knn_neighbor=point_neighbor,memory=memory,two_hop=self.two_hop,point_pred=all_pred,point_W=point_W)
        all_gt_label=memory.labels[-1][point_neighbor.view(-1)].view(len(t_indexes),-1)
        ...
        all_pred_clu,_,_,clu_mapping_0,clu_mapping_1,clu_mapping_2=self.cluster_level_lp(t_indexes,memory.features,self.neighbor_num,ori_0,point_neighbor,f_s=cal_feat, labels=memory.labels[-1],gt_label=all_gt_label,
    ...
    def point_level_merge_split(self,indexes,all_pred,ori_knn_neighbor,memory):
        ...
        for i in range(itera + 1):
            for j in range(memory.iterative):
                memory.labels[j][merge_idx]=memory.labels[j][merge_nei]

        if self.merge_wo_outlier:
            unq_lab,unq_cnt=np.unique(memory.labels[-1].cpu().numpy(),return_counts=True)
            self.outlier_clu=set(unq_lab[unq_cnt<2].tolist())
        return near_neig,merge_idx

    def sub_cluster_level_merge_split(self,indexes,memory,all_pred_sub,ori_knn_neighbor,sub_mapping_0,sub_mapping_1,sub_mapping_2,near_neighbor,merge_idxs,step):
        ...
        for i in range(len(indexes)):
            if self.merge_wo_outlier and indexes[i] not in merge_idxs:
                continue
            lab=memory.labels[step+1][sub_lab[i]]
            merge_idx=set(sub_lab[i][(all_pred_sub[i,0,:len(sub_lab[i])]>bias) & (lab==memory.labels[step+1][indexes[i]])].tolist())
            merge_idx.add(memory.labels[step][indexes[i]].item())
            merge_idx=list(merge_idx)
            if memory.labels[step][near_neighbor[i]].item() not in merge_idx: #reliable neighbor
                continue
            if len(merge_idx)>1:
                merge_label_0=-1
        ...
        for change_label,update_label in merge_map.items():
            memory.labels[step][memory.labels[step]==int(change_label)]=int(update_label)
    ...
    def cluster_level_merge_split(self,indexes,memory,all_pred_clu,ori_knn_neighbor,clu_mapping_0,clu_mapping_1,clu_mapping_2,near_neighbor,merge_idxs):
        ...
        for i in range(len(indexes)):
            ...
            merge_idx.add(memory.labels[-1][indexes[i]].item())
            if self.merge_wo_outlier:
                merge_idx=(merge_idx-self.outlier_clu)
            merge_idx=list(merge_idx)
            if memory.labels[-1][near_neighbor[i]].item() not in merge_idx:
                continue
            ...
            if len(merge_idx)>1:
                ...
                for label in merge_idx:
                    merge_map[label]=merge_label_0
        for change_label,update_label in merge_map.items():
            memory.labels[-1][memory.labels[-1]==int(change_label)]=int(update_label)
...
#others
def p_lp(alpha, method,topk_num):
    model = Point_Level_LP(alpha=alpha,method=method,topk_num=topk_num)
    model.cuda()
...
def c_lp(alpha, topk_num,method,point_wei,connect_num):
    model = Cluster_Level_LP(alpha=alpha,topk_num=topk_num,method=method,point_wei=point_wei,connect_num=connect_num)
    model.cuda()
    #model = nn.DataParallel(model)
    return model
...
def split_lp(alpha,split_num,anchor_thre,gap,connect_num,sp_method):
    model=Split_LP(
        alpha=alpha,
        split_num=split_num,
