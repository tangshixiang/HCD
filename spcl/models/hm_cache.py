...
class HybridMemory(nn.Module):
    def __init__(self, num_features, num_samples, source_classes,source_samples,temp=0.05, momentum=0.2,update_label_thre_max=0.97,update_label_thre_min=0.55,changelabel_thre=0.3,cluster_k1=10,cluster_k2=1):
        ...
        self.register_buffer('label_cache', torch.zeros(num_samples).long())
        self.register_buffer('change_cnt', torch.zeros(num_samples).long())
        self.register_buffer('confidence', torch.zeros(num_samples).long())
    ...
    def forward(self, inputs, indexes,domain=0,gcn_n=None,gcn_s=None):#domain=0:source domain=1:target
        ...
        inputs= hm(inputs, indexes, self.features, domain,self.update_label_thre_max,
            self.update_label_thre_min,self.labels,self.source_classes,self.num_samples,self.momentum,self.changelabel_thre,self.cluster_k1,self.cluster_k2,self.change_cnt,self.label_cache,self.confidence,gcn_n,gcn_s)
