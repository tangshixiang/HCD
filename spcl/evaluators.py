...
def extract_cnn_feature_1(model, inputs,idx):
    inputs = to_torch(inputs).cuda()
    outputs = model(inputs,mode='extract_reid')
    #import pdb;pdb.set_trace()
    outputs = outputs.data.cpu()
    return outputs

def extract_cnn_feature_2(model, inputs):
    inputs = to_torch(inputs).cuda()
...
def extract_features_1(model, data_loader, print_freq=50):
    model.eval()
    batch_time = AverageMeter()
    data_time = AverageMeter()
    ...
    with torch.no_grad():
        for i, (imgs, fnames, pids, _, _) in enumerate(data_loader):
            ...
            outputs = extract_cnn_feature_1(model, imgs,i)
            for fname, output, pid in zip(fnames, outputs, pids):
                features[fname] = output
                labels[fname] = pid
...
def extract_features_2(model, data_loader, print_freq=50):
    model.eval()
    batch_time = AverageMeter()
    data_time = AverageMeter()
    ...
    with torch.no_grad():
        for i, (imgs, fnames, pids, _, _) in enumerate(data_loader):
            ...
            outputs = extract_cnn_feature_2(model, imgs)
            for fname, output, pid in zip(fnames, outputs, pids):
                features[fname] = output
                labels[fname] = pid
...
class Evaluator_1(object):
    def __init__(self, model):
        super(Evaluator_1, self).__init__()
        self.model = model

    def evaluate(self, data_loader, query, gallery, cmc_flag=False, rerank=False):
        features, _ = extract_features_1(self.model, data_loader)
        distmat, query_features, gallery_features = pairwise_distance(features, query, gallery)
        results = evaluate_all(query_features, gallery_features, distmat, query=query, gallery=gallery, cmc_flag=cmc_flag)
...
class Evaluator_2(object):
    def __init__(self, model):
        super(Evaluator_2, self).__init__()
        self.model = model

    def evaluate(self, data_loader, query, gallery, cmc_flag=False, rerank=False):
        features, _ = extract_features_2(self.model, data_loader)
        distmat, query_features, gallery_features = pairwise_distance(features, query, gallery)
        results = evaluate_all(query_features, gallery_features, distmat, query=query, gallery=gallery, cmc_flag=cmc_flag)
